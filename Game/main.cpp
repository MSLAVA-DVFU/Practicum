...
int main() {
    ...
    for (int y = 1; y < HEIGHT_MAP-1; y++) {
        for (int x = 1; x < WIDTH_MAP-1; x++) {
            if (TileMap[y][x] != '0' && TileMap[y+1][x] != '0' && TileMap[y][x+1] != '0' && TileMap[y-1][x] != '0' && TileMap[y][x-1] != '0') { // Если не стена
                freePositions.push_back(sf::Vector2f(x, y));
            }
        }
    }
    // Случайный выбор из доступных позиций
    int randomIndex = rand() % (freePositions.size());

    Player* player = new Player(textures::player_texture, sf::Vector2f(freePositions[randomIndex].x*32, freePositions[randomIndex].y*32), 100, 0, true,1);
    HealthBar healthBar(10, 10, 200, 20, player->getHP());
    ...
    while (window.isOpen()) {
        ...
        if (!player->m_life || player->m_keys==3) {
            isGameOver = true; // Игра окончена, показываем окно результатов
            player->m_life = false;
        }
        ...
        if (isGameOver) {
            ...
            window.display(); // Обновляем экран и пропускаем остальные действия
            continue;
        }
