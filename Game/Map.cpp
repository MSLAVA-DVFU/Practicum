...
std::vector<std::pair<int, int>> generateLevel() {
    ...
    const int roomSize = 7;  // Размер комнаты (например, 3x3)
    ...
    for (int i = 0; i < roomCount; i++) {
        ...
        while (!roomPlaced && attempts < 100) {
            ...
            if (isSpaceAvailable) {
                ...
                int itemX = roomX + (rand() % roomSize) + 1; // Случайная позиция внутри комнаты по X
                int itemY = roomY + (rand() % roomSize) + 1; // Случайная позиция внутри комнаты по Y
